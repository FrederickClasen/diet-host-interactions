%% PATHS
addpath(genpath('/Users/clasenf/OneDrive - The Fra ncis Crick Institute/Integration'));
...
load('GEM/multiTissueModels.mat');
load('data/diet.mat');
...
for i=1:length(models)
    ...
    tissue_objectives = {'t_MMRN_Biomass';'l_MMRN_Biomass'};
...
for i=1:length(models)
    ...
    %tissue objectives
    %tissue_objectives = model.rxns(find(contains(string(model.rxns),"MMRN_Biomass")));
    tissue_objectives = {'l_MMRN_Biomass'};
...
experiment = 'SIM4';
% RER AND OBJECTIVES
RERRxns = {'EXC_IN_C00007[s]','EXC_OUT_C00011[s]'}; % O2 uptake and CO2 production    
objective = {'MMRN_Biomass'};          % OBJECTIVE FUNCTION

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% load diet information
conditions = {'DEN_Tumour_fed','DEN_Tumour_fasted'};
%CD = importExcelModel('GEM/Liver/genericLiverLFD.xlsx',false);
%WD = importExcelModel('GEM/Liver/genericLiverWD.xlsx',false);
models = {CD,WD};
fluxVector = zeros(length(WD.rxns),(length(conditions)*2)); 
diets = {'CD','WD'};
modelNames = cell(0,length(conditions) * 2);
...
for i=1:length(diets)
    ...
    for j=1:length(conditions)
        counter = counter + 1;
        fc = 'GEM/Eflux/' + string(conditions(j)) + '.csv';
        ...
        modelNames{counter} = char(modelName);
...
T.Properties.VariableNames = colheads;
writetable(T,'output/FBA/'+string(experiment)+'.xlsx','Sheet',experiment,'WriteVariableNames',true);
